...
function scrapeTable($url, $startMarker) {
    $csv_data = '';
    ...
    function sqlRaw__handleLink($url, $source='csvfile', $startMarker){
        global $ID;
        $delim = ',';
        $opt = array('content' => '');
        ...
        if ($source == 'csvfile') {
            ...
            $content =& scrapeTable($url, $startMarker);
        } else {
    ...
    function render($mode, &$renderer, $data) {
        ...
        if($mode == 'xhtml'){		
           ...
           if ($data['class'] != FALSE) 
			...
			if ($data['link'] != "") {
				$db =& DB::connect('mysql://glocal6_tempDb:e*i@KO9^6J@*@localhost/glocal6_temp');
				if (DB::isError($db)) {
					$error = $db->getMessage();
    			...
    			$theResult = sqlRaw__handleLink($data['link'], $this->source, $this->startMarker);
    			if ($theResult != "") {
